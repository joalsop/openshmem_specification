...
Section~\ref{subsubsec:api_observable} details how operations trigger observable memory accesses, but it does not define how the ordering of these accesses are constrained in an execution. This section defines a set of API-level relations between normal memory accesses, API operations, and observable accesses issued by these operations.
In the same way that the PL-level \relHB{} relation is used to constrain ordering between normal memory accesses, these new API relations establish similar ordering constraints between normal memory accesses and observable accesses, and between observable accesses and other observable accesses.
Table~\ref{table:api_relations} summarizes what types of accesses that each API-level relation may connect, and which \openshmem operations may establish it.
In Section~\ref{subsubsec:api_integrate}, these new relations are then used to extend the definition of \relHB{} and \relDR{} for the API memory model.

\normalsize
\begin{longtable}{|l|>{\centering\arraybackslash}p{0.20\textwidth}|>{\centering\arraybackslash}p{0.22\textwidth}|p{0.38\textwidth}|}
\hline
\textbf{API Relation} & \textbf{Normal $\leftrightarrow$ Observable} & \textbf{Observable $\leftrightarrow$ Observable} & \textbf{Established by:}
\tabularnewline\hline
\endhead
...
\relRCO{} & X & X & Quiet and barrier operations \tabularnewline\hline
